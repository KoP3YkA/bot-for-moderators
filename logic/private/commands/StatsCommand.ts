...
export class StatsCommand extends BaseExecutor {
    ...
    public async execute(message: PrivateMessageEvent) : Some {
        ...
        if (target.moderatorType === ModeratorType.PANEL && sender.rank !== ModeratorRank.CHIEF) return await message.reply(`С другим составом может взаимодействовать только главный модератор!`)
